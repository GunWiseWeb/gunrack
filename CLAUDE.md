...
11. **Application `get__icon()` must have `: string` return type** — the parent `\IPS\Application::get__icon()` signature declares `: string`, so overrides must match: `protected function get__icon(): string { return 'database'; }`. Omitting the return type causes a fatal LSP-violation error at class-load time and the ACP will white-screen.
