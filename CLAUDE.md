...
4. **Do NOT use `\IPS\Theme::i()->getTemplate()` in controllers** — output HTML via `\IPS\Output::i()->output = '<html string>'` with inline HTML. Templates are seeded into `core_theme_templates` directly via `setup/install.php` using `\IPS\Db::i()->insert()` (bypassing IPS's XML import, which corrupts `{{-- comments --}}` to bare `-- comments --` and breaks template eval). Do NOT use `data/theme.xml` — it is not packaged.
