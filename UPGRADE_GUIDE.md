...
- **Email templates must be in `data/emails.xml`, not only in the PHP EmailTemplates extension** — IPS loads email templates from `core_email_templates` table, which is populated from `data/emails.xml` during install. The PHP `EmailTemplates` extension class is only used in dev mode. Without `emails.xml`, `\IPS\Email::buildFromTemplate()` throws "Template not found" in production. On upgrade, call `$app->installEmailTemplates()` in the upgrade step to seed them.
- **Never share a try/catch between `\IPS\Email::buildFromTemplate()` and `new \IPS\Notification()`** — if the email call throws (template missing, SES error, invalid member) the shared outer catch swallows it and the notification block is never reached. The inline notification silently vanishes. Always give email, inline notification, and PM each their OWN top-level try/catch block, non-nested, so one channel's failure cannot suppress another.
...
- `10017` — 1.0.17 — Re-added `use IPS\Member;` to `DealerNotifications.php`. The `configurationOptions( ?Member $member = NULL )` signature still references `Member` as a typehint; without the import PHP resolves it to `IPS\gddealer\extensions\core\Notifications\Member` and throws a fatal error that prevents all notifications from loading. Upgrade step also patches `extensions.json` and `DealerNotifications.php` on disk for installs where tar file deployment didn't land the fixed versions. No schema changes.
- `10018` — 1.0.18 — Added `data/emails.xml` with all 11 email templates (newDealerReview, dealerWelcome, trialExpired, trialExpiringSoon, disputeNotify, disputeAdminNotify, disputeDismissed, disputeAutoResolved, disputeUpheld, disputeOutcome, dealerResponded). IPS populates `core_email_templates` from this XML file during install; the PHP `EmailTemplates` extension is only used in dev mode. `upg_10018/upgrade.php` calls `$app->installEmailTemplates()` to seed them for existing installs. No schema changes.
- `10019` — 1.0.19 — Isolated email, inline notification, and PM side-effects into independent try/catch blocks at every notification site. A failed email (missing template, SES error, invalid recipient) was being caught by the outer try that also contained `new \IPS\Notification(...)` / `Conversation::createItem(...)`, silently suppressing the inline notification and PM. Fixed in six call sites: `rate()` + `disputeRespond()` in `modules/front/dealers/profile.php`, `respond()` + `dispute()` in `modules/front/dealers/dashboard.php`, `upholdDispute()` + `dismissDispute()` in `modules/admin/dealers/dealers.php`. `upg_10019/upgrade.php` re-runs `$app->installEmailTemplates()` defensively. No schema changes.
- `10020` — 1.0.20 — Added "review updated" notification. When a reviewer edits an existing review via `rate()`, the dealer now receives a distinct email (`updatedDealerReview`) and inline notification (`updated_dealer_review`) instead of the new-review ones. New `parse_updated_dealer_review()` method and `updated_dealer_review` config entry in `DealerNotifications.php`. New `updatedDealerReview` template in `data/emails.xml`. New lang keys: `gddealer_notif_updated_review`, `gddealer_notif_updated_review_desc`, `mailsub__gddealer_notification_updated_dealer_review`, `mailani__gddealer_notification_updated_dealer_review`. `upg_10020/upgrade.php` seeds the notification default and re-runs `installEmailTemplates()`. No schema changes.
- `10021` — 1.0.21 — Self-heal for installs where `core_email_templates` was left empty because IPS's `installEmailTemplates()` never actually ran from `data/emails.xml`. `upg_10021/upgrade.php` parses `data/emails.xml` directly with SimpleXML and inserts any missing row into `core_email_templates` (per-template try/catch with a minimal-column fallback for schema variants); also idempotently re-seeds all seven gddealer notification defaults in `core_notification_defaults` and clears the `extensions`, `applications`, and `emailTemplates` datastore keys. The same XML-parsing seeder is now in `setup/install.php` too, so fresh installs no longer depend on IPS auto-seeding from `emails.xml`. No schema changes.
