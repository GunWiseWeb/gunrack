...
- `10007` — 1.0.7 — Admin review deletion and PM-to-reviewer on dealer/admin actions. Adds `deleteReview()` and standalone `reviews()` (All Reviews) ACP actions; Recent Reviews table on dealer detail view with per-row delete buttons. `respond()` in the dealer dashboard sends a PM + `dealer_responded` IPS notification to the reviewer. `upholdDispute()` and `dismissDispute()` send a PM to the reviewer with the outcome. `dispute()` sends a PM to the customer alongside the existing email + notification. New acpmenu entry `reviews`, new `allReviews` admin template. `setup/templates_10007.php` syncs the updated `dealerDetail` template and installs the new `allReviews` template. No schema changes.
