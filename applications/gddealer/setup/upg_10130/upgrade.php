<?php
namespace IPS\gddealer\setup\upg_10130;
...
class _upgrade
{
    public function step1(): bool
    {
        ...
        /* CRITICAL: Restore dealerShell to canonical body FIRST. The wrapping
           templates (10106, 10107_shell, 10108_shell) stacked CSS/wrapper divs
           on every upgrade, inflating dealerShell to 465KB and crashing
           IPS\Theme->getTemplate() with UnexpectedValueException. */
        try { require_once \IPS\ROOT_PATH . '/applications/gddealer/setup/templates_10130.php'; }
        catch ( \Throwable $e ) { $errors[] = 'templates_10130.php (dealerShell restore) failed: ' . $e->getMessage(); }

        /* Force IPS to recompile templates from the restored source. The
           existing 838KB compiled cache file is unparseable. */
        try
        {
            \IPS\Db::i()->update( 'core_themes', [ 'set_cache_key' => 'rb' . time() ] );
        ...
        {
            $errors[] = 'Cache invalidation failed: ' . $e->getMessage();
        }

        /* Seed v10128/v10129 lang strings. */
        $feedLangStrings = [
        ...
        {
            $langIds = [];
            foreach ( \IPS\Db::i()->select( 'lang_id', 'core_sys_lang' ) as $lid ) { $langIds[] = (int) $lid; }
            foreach ( $langIds as $lid )
            {
                foreach ( $feedLangStrings as $key => $val )
                {
                    try
                    {
        ...
        {
            $errors[] = 'Feed lang seed v10130 failed: ' . $e->getMessage();
        }
        ...
        /* Re-run the template seed chain for consistency.
           NOTE: templates_10106, templates_10107_shell, templates_10108_shell
           are INTENTIONALLY excluded. They were the wrapping scripts that caused
           the 465KB dealerShell crash. They now have idempotency guards and
           should NOT be re-run after templates_10130 restores canonical. */
        ...
        try { require_once \IPS\ROOT_PATH . '/applications/gddealer/setup/templates_10127.php'; }
        catch ( \Throwable $e ) { $errors[] = 'templates_10127.php (badge card de-dup) failed: ' . $e->getMessage(); }

        try { require_once \IPS\ROOT_PATH . '/applications/gddealer/setup/templates_10128.php'; }
        catch ( \Throwable $e ) { $errors[] = 'templates_10128.php (manual feed upload panel) failed: ' . $e->getMessage(); }

        try { require_once \IPS\ROOT_PATH . '/applications/gddealer/setup/templates_10129.php'; }
        catch ( \Throwable $e ) { $errors[] = 'templates_10129.php (upload panel rewrite) failed: ' . $e->getMessage(); }

        try
        {
            $existingConfig = (int) \IPS\Db::i()->select( 'COUNT(*)', 'core_file_storage', [
                'method=? OR configuration LIKE ?', 'FileSystem', '%gddealer_FeedUpload%'
            ] )->first();
        }
        catch ( \Throwable ) {}
        ...
        try { \IPS\Log::log( $errors ? implode( "\n", $errors ) : 'v1.0.130 upgrade completed cleanly', 'gddealer_upg_10130' ); }
        catch ( \Throwable ) {}
