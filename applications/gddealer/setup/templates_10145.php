...
/**
 * v1.0.145 - Add 'profile' icon to dealerNavIcon template.
 *
 * The Account section's "Edit Profile" item uses 'icon' => 'profile' but the
 * icon template has no matching branch, so the SVG silently fails to render.
 * This seed file rewrites the whole template (per project rule #38) to
 * include all 10 existing icons (the 9 originals plus the v144 validator)
 * PLUS a new profile icon (user silhouette in a circle).
 *
...
{
	try { \IPS\Log::log( 'templates_10145.php update failed: ' . $e->getMessage(), 'gddealer_upg_10145' ); }
	catch ( \Throwable ) {}
}
