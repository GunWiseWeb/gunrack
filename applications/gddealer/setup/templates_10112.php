...
/* Strip the v10111 FFL section (wrong CSS class names, broken layout). */
if ( strpos( $body, 'GD_FFL_SECTION_INJECTED_v10111' ) !== false )
{
	/* The v10111 section is bracketed by the marker comment above and a closing
	   </div> at the bottom of its outermost div. Match greedy from marker to end
	   of the outer gdFflSection div. */
	$body = preg_replace(
		'#<!-- GD_FFL_SECTION_INJECTED_v10111 -->.*?(?=<div class="gdPanel")#s',
		'',
		$body
	);
}

$marker = '<!-- GD_FFL_SECTION_INJECTED_v10112 -->';
if ( strpos( $body, $marker ) !== false ) { return; }

$fflSection = <<<'HTML'
<!-- GD_FFL_SECTION_INJECTED_v10112 -->
...
/* Inject above the first gdPanel (the Identity section). Falls back to form-start. */
