...
TEMPLATE_EOT,
	...
	[
		...
		'group'         => 'dashboard',
		'template_name' => 'dashboard',
		'template_data' => '$tab, $overview, $feed, $listings, $unmatchedRows, $logs, $tabUrls, $importUrl',
		'template_content' => <<<'TEMPLATE_EOT'
<div class="ipsBox">
	<h1 class="ipsBox_title">{lang="gddealer_frontend_dashboard_title"}</h1>
	<div class="ipsPad">

		<nav class="ipsTabBar" style="margin-bottom:16px">
			<a href="{$tabUrls['overview']}" class="ipsTabBar_tab {{if $tab === 'overview'}}ipsTabBar_tab--active{{endif}}">{lang="gddealer_frontend_overview"}</a>
			<a href="{$tabUrls['feed']}" class="ipsTabBar_tab {{if $tab === 'feed'}}ipsTabBar_tab--active{{endif}}">{lang="gddealer_frontend_feed_settings"}</a>
			<a href="{$tabUrls['listings']}" class="ipsTabBar_tab {{if $tab === 'listings'}}ipsTabBar_tab--active{{endif}}">{lang="gddealer_frontend_listings"}</a>
			<a href="{$tabUrls['unmatched']}" class="ipsTabBar_tab {{if $tab === 'unmatched'}}ipsTabBar_tab--active{{endif}}">{lang="gddealer_frontend_unmatched"}</a>
			<a href="{$tabUrls['analytics']}" class="ipsTabBar_tab {{if $tab === 'analytics'}}ipsTabBar_tab--active{{endif}}">{lang="gddealer_frontend_analytics"}</a>
			<a href="{$tabUrls['subscription']}" class="ipsTabBar_tab {{if $tab === 'subscription'}}ipsTabBar_tab--active{{endif}}">{lang="gddealer_frontend_subscription"}</a>
		</nav>

		{{if $tab === 'overview'}}
			<div style="display:flex;gap:16px;margin-bottom:24px;flex-wrap:wrap">
				<div class="ipsBox" style="flex:1 1 180px;padding:16px;text-align:center">
					<div style="font-size:1.6em;font-weight:bold">{$overview['active_listings']}</div>
					<div>Active Listings</div>
				</div>
				<div class="ipsBox" style="flex:1 1 180px;padding:16px;text-align:center">
					<div style="font-size:1.6em;font-weight:bold">{$overview['oos_listings']}</div>
					<div>Out of Stock</div>
				</div>
				<div class="ipsBox" style="flex:1 1 180px;padding:16px;text-align:center">
					<div style="font-size:1.6em;font-weight:bold">{$overview['unmatched_count']}</div>
					<div>Unmatched UPCs</div>
				</div>
				<div class="ipsBox" style="flex:1 1 180px;padding:16px;text-align:center">
					<div style="font-size:1.6em;font-weight:bold">{$overview['clicks_7d']}</div>
					<div>Clicks (7d)</div>
				</div>
				<div class="ipsBox" style="flex:1 1 180px;padding:16px;text-align:center">
					<div style="font-size:1.6em;font-weight:bold">{$overview['clicks_30d']}</div>
					<div>Clicks (30d)</div>
				</div>
			</div>

			<h2>Last Feed Import</h2>
			<p>
				{{if $overview['last_run']}}
					{$overview['last_run']} &mdash; {$overview['last_run_status']} &mdash; {$overview['last_record_count']} records
				{{else}}
					<em>No imports yet. Add your feed URL in the Feed Settings tab.</em>
				{{endif}}
			</p>

		{{elseif $tab === 'feed'}}
			<p><strong>Feed URL:</strong> <code>{$feed['feed_url']}</code></p>
			<p><strong>Format:</strong> {$feed['feed_format']}</p>
			<p><strong>Schedule:</strong> {$feed['import_schedule']}</p>
			<p><strong>Last Run:</strong> {$feed['last_run']}</p>
			<p style="margin-top:16px">
				<a href="{$importUrl}" class="ipsButton ipsButton--primary">Run Import Now</a>
			</p>

			<h2 style="margin-top:24px">Import History</h2>
			<table class="ipsTable ipsTable_zebra" style="width:100%">
				<thead>
					<tr><th>Started</th><th>Status</th><th>Total</th><th>New</th><th>Updated</th><th>Unmatched</th></tr>
				</thead>
				<tbody>
					{{foreach $logs as $l}}
					<tr>
					...
					{{endforeach}}
					{{if count( $logs ) === 0}}
					<tr><td colspan="6" style="text-align:center;color:#999;padding:24px">No imports yet.</td></tr>
					{{endif}}
				</tbody>
			</table>
		...
		{{elseif $tab === 'listings'}}
			<table class="ipsTable ipsTable_zebra" style="width:100%">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							{{if $l['in_stock']}}
								<span class="ipsBadge ipsBadge--positive">In Stock</span>
							{{else}}
								<span class="ipsBadge ipsBadge--neutral">Out of Stock</span>
							{{endif}}
						</td>
						<td>{$l['condition']}</td>
						<td>{$l['last_updated']}</td>
					...
					{{endforeach}}
					{{if count( $listings ) === 0}}
					<tr><td colspan="5" style="text-align:center;color:#999;padding:24px">No listings yet. Run your first feed import from the Feed Settings tab.</td></tr>
					{{endif}}
				</tbody>
			</table>

		{{elseif $tab === 'unmatched'}}
			<p>These UPCs appeared in your feed but have no match in our master catalog. GunRack admins review these and add new catalog entries.</p>
			<table class="ipsTable ipsTable_zebra" style="width:100%">
				<thead>
					<tr><th>UPC</th><th>First Seen</th><th>Last Seen</th><th>Times Seen</th></tr>
				</thead>
				<tbody>
					...
					<tr><td colspan="4" style="text-align:center;color:#999;padding:24px">No unmatched UPCs.</td></tr>
					{{endif}}
				</tbody>
			</table>

		{{elseif $tab === 'analytics'}}
			{{if $overview['tier'] === 'pro' || $overview['tier'] === 'enterprise'}}
				<p>Analytics available for Pro and Enterprise tiers.</p>
				<p><strong>Last 7 days clicks:</strong> {$overview['clicks_7d']}</p>
				<p><strong>Last 30 days clicks:</strong> {$overview['clicks_30d']}</p>
			{{else}}
				<div class="ipsMessage ipsMessage--info">
					<p>Analytics are available on the Pro and Enterprise tiers. Upgrade your subscription to see click-through analytics, price competitiveness, and revenue opportunity indicators.</p>
				</div>
			{{endif}}

		{{elseif $tab === 'subscription'}}
			<p><strong>Current Tier:</strong> {$overview['tier']}</p>
			<p>Billing, upgrades, and cancellation are handled through your IPS account subscriptions.</p>
		{{endif}}
