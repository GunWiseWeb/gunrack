...
$gddealerV10076Templates = [];

/* reviews — redesigned cards with avatar, verified buyer, product link, contest block, timeline */
$gddealerV10076Templates[] = [
    'set_id'            => 1,
    'app'               => 'gddealer',
    'location'          => 'front',
...
<div class="gdReviewList">
    ...
    <article class="gdReviewCard {expression="$r['dispute_status'] !== 'none' ? 'gdReviewCard--contested' : ''"}">
        ...
        <div class="gdReviewCard__contestBanner">
            ...
            <span>{$r['dispute_status_label']}</span>
            {{if $r['deadline_days_left'] >= 0}}
            <span class="gdReviewCard__deadlineBadge">{$r['deadline_days_left']}d left</span>
            {{endif}}
        </div>
        {{endif}}
        ...
        {{if $r['dispute_status'] !== 'none'}}
        <div class="gdReviewCard__contestSection">
            {{if $r['dispute_reason']}}
            ...
            <div class="gdContestBlock gdContestBlock--customer">
                ...
                <details class="gdContestBlock__evidence">
                    <summary>View customer evidence</summary>
            ...
            <div class="gdContestBlock__nextSteps">
                ...
                The customer has until {$r['deadline_display']} to respond. If they don't reply, the dispute will be automatically resolved in your favor.
            </div>
            {{elseif $r['dispute_status'] === 'pending_admin'}}
            <div class="gdContestBlock__nextSteps">
                <strong>Under review</strong>
                An admin is reviewing this dispute. Both parties have submitted their positions. You'll be notified when a decision is made.
            </div>
            {{endif}}

            {{if $r['dispute_reason_editor_html']}}
        ...
        {{if count($r['activity']) > 1}}
        <div class="gdTimeline">
            <div class="gdTimeline__label">Activity</div>
            {{foreach $r['activity'] as $ev}}
            <div class="gdTimeline__item">
                <span class="gdTimeline__dot gdTimeline__dot--{$ev['color']}"></span>
                <span class="gdTimeline__text">{$ev['label']}</span>
                <span class="gdTimeline__time">{$ev['time']}</span>
            </div>
            {{endforeach}}
        </div>
        {{endif}}
...
foreach ( $gddealerV10076Templates as $tpl ) {
