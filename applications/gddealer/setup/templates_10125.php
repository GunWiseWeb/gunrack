...
/* Strip the v10124 injection (everything from its marker to end of body). */
if ( strpos( $body, 'GD_VERIFIED_BADGE_CARD_v10124' ) !== false )
{
	$body = preg_replace( '#<!-- GD_VERIFIED_BADGE_CARD_v10124 -->.*$#s', '', $body );
}

$marker = '<!-- GD_VERIFIED_BADGE_CARD_v10125 -->';
if ( strpos( $body, $marker ) !== false ) { return; }

/* Markup-only injection — JS is now loaded inline by DealerShellTrait::output()
   reading from dev/js/front/badge-picker.js. No <script> tag in this template. */
$badgeCard = <<<'HTML'
