...
/**
 * v1.0.135 — Restores the dealerDirectory layout that v1.0.134 incorrectly
 * stripped out, plus widens the container to 1446px to match the rest of
 * the site.
 *
 * History:
 *   - v1.0.132 attempted a redesign but shipped with class-name and field-
 *     name mismatches between the markup and the inline CSS, so the page
 *     rendered identical to the old design.
 *   - v1.0.133 fixed those bugs and shipped the working redesign with sort
 *     dropdown, grid/list toggle, and 1446px width.
 *   - v1.0.134 misdiagnosed user feedback and REMOVED the sort dropdown,
 *     grid/list toggle, and reverted width to 1200px.
 *   - v1.0.135 (this version) restores everything v1.0.133 shipped.
 *
 * IMPORTANT FOR THE BUILDER: Do NOT remove any of these features even if
 * they appear "decorative" or "non-functional" in static HTML inspection.
 * They ARE functional:
 *   - Sort dropdown submits via the wrapping <form method="get"> with
 *     onchange="this.form.submit()". The directory.php controller reads
 *     ?sort= and orders the dealer list accordingly.
 *   - Grid/List view toggle has working JS at the bottom of the template
 *     that toggles a CSS class on the grid container and persists the
 *     choice in localStorage.
 *   - The 1446px max-width matches the rest of the gunrack.deals site.
 *
 * Tier filter chips ARE intentionally absent (tier filtering was dropped
 * by user request). Do not add them back.
 *
 * Following project rule #28: full template body declared inline as a
 * nowdoc heredoc, no escape sequences in template body.
 * Following project rule #19: update() to overwrite existing row.
 */
...
{
	try { \IPS\Log::log( 'templates_10135.php update failed: ' . $e->getMessage(), 'gddealer_upg_10135' ); }
	catch ( \Throwable ) {}
}
