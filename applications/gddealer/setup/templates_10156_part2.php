...
/**
 * v1.0.156 PART 2 of 2 - Append step 4 CSS + persist setupWizardStep4
 * template to database. Same CSS as v155 part 4 (unchanged).
 *
 * Must run after part 1. Uses UPDATE only (the row was created in v155).
 */

if ( !isset( $step4Tpl ) )
{
    throw new \RuntimeException( 'templates_10156_part2.php loaded before part1' );
}
...
.gdSetupWizard__step4Stats { display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; }
...
.gdSetupWizard__statusBadge { display: inline-block; padding: 3px 10px; border-radius: 12px; font-size: 11.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.4px; }
...
.gdSetupWizard__details summary { cursor: pointer; color: #475569; font-weight: 600; padding: 2px 0; }
...
@media (max-width: 1100px) { .gdSetupWizard__step4Stats { grid-template-columns: 1fr 1fr; } }
...
{
    try { \IPS\Log::log( 'templates_10156_part2.php failed: ' . $e->getMessage(), 'gddealer_upg_10156' ); }
    catch ( \Throwable ) {}
}
