...
/**
 * v1.0.155 PART 3 of 6 - HTML body of setupWizardStep4.
 *
 * Validate Sample. Reads cached step 4 report from controller and
 * renders:
 *   - Summary stats (X valid / Y errors / Z warnings)
 *   - Per-row table with status badge, raw → canonical preview, and
 *     expandable details for any errors/warnings
 *   - Continue button locked until at least 1 record is valid
 */
...
<div class="gdSetupWizard">
	...
	{{else}}
		...
		<section class="gdSetupWizard__card">
			<div class="gdSetupWizard__step4Stats">
				...
				<div class="gdSetupWizard__stat gdSetupWizard__stat--{{if $summary['error_records'] > 0}}bad{{else}}neutral{{endif}}">
					<span class="gdSetupWizard__statNum">{$summary['error_records']}</span>
					<span class="gdSetupWizard__statLabel">With errors</span>
				</div>
				<div class="gdSetupWizard__stat gdSetupWizard__stat--{{if $summary['warning_records'] > 0}}warn{{else}}neutral{{endif}}">
	...
	<form method="post" action="{$values['urls']['save_step4']}" class="gdSetupWizard__form" data-step="4">
		...
		<div class="gdSetupWizard__actions">
			...
			{{if $values['report'] !== null && $values['report']['summary']['valid_records'] > 0}}
