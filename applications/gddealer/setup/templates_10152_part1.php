...
/**
 * v1.0.152 PART 1 of 3 - Replace setupWizardStep1 template with version
 * that uses {$values['urls']['key']} string interpolation instead of
 * {url=...} template directives.
 *
 * The {url=...} directive's nested quoting was breaking HTML attribute
 * parsing - inner quotes terminated the outer quotes, leaving literal
 * "{url=app=gddealer..." text in form action= attributes.
 *
 * Fix: build all URLs in the controller, pass as $values['urls']['key'],
 * use simple {$values['urls']['key']} interpolation in the template.
 *
 * Same applies to the CSRF key - now passed as $values['csrfKey'] string.
 */
...
<div class="gdSetupWizard">
	...
	<form method="post" action="{$values['urls']['save_step1']}" class="gdSetupWizard__form" data-step="1">
		...
		<section class="gdSetupWizard__card gdSetupWizard__urlMode" data-mode-pane="url">
			...
			<label class="gdSetupWizard__field">
				...
				<span class="gdSetupWizard__fieldHelp">Leave blank if Auth Type is None. For Basic Auth: <code>{"username":"u","password":"p"}</code>. For API Key: <code>{"api_key":"k"}</code>.</span>
			</label>
		</section>
...
.gdSetupWizard__card { background: #fff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 20px; }
.gdSetupWizard__card h3 { margin: 0 0 8px; font-size: 1.1em; font-weight: 700; }
.gdSetupWizard__card h4 { margin: 0 0 8px; font-size: 1em; font-weight: 600; }
.gdSetupWizard__card p { margin: 0 0 12px; color: #475569; font-size: 14px; }
.gdSetupWizard__card p:last-child { margin-bottom: 0; }

.gdSetupWizard__modeToggle { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-top: 12px; }
.gdSetupWizard__modeOption { display: flex; flex-direction: column; gap: 4px; padding: 14px 16px; border: 2px solid #e2e8f0; border-radius: 10px; cursor: pointer; transition: all 0.15s; }
.gdSetupWizard__modeOption:has(input:checked) { border-color: #2563eb; background: #eff6ff; }
.gdSetupWizard__modeOption input { margin-right: 8px; }
.gdSetupWizard__modeLabel { display: inline-block; font-weight: 600; font-size: 14px; color: #0f172a; }
.gdSetupWizard__modeHint { font-size: 12.5px; color: #64748b; line-height: 1.45; }

.gdSetupWizard__field { display: flex; flex-direction: column; gap: 6px; margin-bottom: 14px; }
.gdSetupWizard__field:last-child { margin-bottom: 0; }
.gdSetupWizard__fieldLabel { font-size: 13px; font-weight: 600; color: #0f172a; }
.gdSetupWizard__fieldHelp { font-size: 12.5px; color: #64748b; line-height: 1.45; }
.gdSetupWizard__fieldHelp code { background: #f1f5f9; padding: 1px 6px; border-radius: 3px; font-family: 'JetBrains Mono', ui-monospace, monospace; font-size: 11.5px; }
.gdSetupWizard__req { color: #dc2626; }

.gdSetupWizard__input, .gdSetupWizard__select, .gdSetupWizard__textarea { width: 100%; padding: 9px 12px; border: 1px solid #cbd5e1; border-radius: 6px; font-family: inherit; font-size: 14px; color: #0f172a; background: #fff; }
.gdSetupWizard__input:focus, .gdSetupWizard__select:focus, .gdSetupWizard__textarea:focus { outline: none; border-color: #2563eb; box-shadow: 0 0 0 3px rgba(37, 99, 235, 0.1); }
.gdSetupWizard__input--mono, .gdSetupWizard__textarea--mono { font-family: 'JetBrains Mono', ui-monospace, monospace; font-size: 12.5px; }
.gdSetupWizard__textarea { resize: vertical; min-height: 240px; line-height: 1.5; }

.gdSetupWizard__actions { display: flex; justify-content: flex-end; gap: 8px; padding-top: 8px; }
...
{
    try { \IPS\Log::log( 'templates_10152_part1.php failed: ' . $e->getMessage(), 'gddealer_upg_10152' ); }
    catch ( \Throwable ) {}
}
