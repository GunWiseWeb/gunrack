...
/**
 * v1.0.134 — Strip non-functional UI and fix search.
 *
 * Changes vs v1.0.133:
 *   - Removed tier filter chips (didn't work)
 *   - Removed sort-by dropdown (didn't work)
 *   - Removed Grid/List view toggle (decorative only)
 *   - Added hidden inputs for app/module/controller routing params so the
 *     search form GET submission survives FURL stripping. The browser strips
 *     the action URL's query string on form GET, so the routing must be in
 *     hidden inputs to reach the gddealer directory controller instead of
 *     falling back to /index.php and the default app.
 *   - Added a Clear button next to Search when a search term is active.
 *   - Results meta now shows the search term when one is active.
 *
 * Following project rule #28: nowdoc heredoc for template body.
 * Following project rule #19: replace via update() to overwrite existing row.
 */
...
.gd-directory .search-clear { padding: 12px 16px; border-radius: var(--gd-r-lg); background: var(--gd-surface); color: var(--gd-text-muted); border: 1px solid var(--gd-border); font-family: inherit; font-size: 14px; font-weight: 500; cursor: pointer; transition: all 0.15s; white-space: nowrap; text-decoration: none; display: inline-flex; align-items: center; }
.gd-directory .search-clear:hover { background: var(--gd-surface-muted); border-color: var(--gd-border-strong); color: var(--gd-text); }
...
@media (max-width: 640px) {
    ...
    .gd-directory .search-submit, .gd-directory .search-clear { width: 100%; justify-content: center; }
}
</style>
    ...
    <div class="directory-hero">
        ...
        <div class="search-row">
            ...
            {{if $search}}
            <a href="{$directoryUrl}" class="search-clear">Clear</a>
            {{endif}}
        </div>
    </div>
    </form>
    ...
    <div class="results-meta">
        <div class="results-count">
            {{if $search}}
                <strong>{$total} dealers</strong> matching &ldquo;{$search}&rdquo;
            {{else}}
                <strong>{$total} dealers</strong>
            {{endif}}
        </div>
    </div>
...
{{if count($dealers) === 0}}
    <div class="gd-empty">
        ...
        <p>Try a different search term, or clear the search to see all dealers.</p>
    </div>
{{else}}
...
{{endif}}
    ...
    <div class="join-banner">
        <div class="join-banner-content">
            ...
            <p class="join-banner-sub">Join gunrack.deals to list your inventory, reach thousands of buyers, and grow your business.</p>
        </div>
...
{
	try { \IPS\Log::log( 'templates_10134.php update failed: ' . $e->getMessage(), 'gddealer_upg_10134' ); }
	catch ( \Throwable ) {}
}
